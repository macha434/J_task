...
int
niusb6501_is_success(size_t len, 
		     const void *buffer)
{
  static const char success_packet[] = "\x00\x08\x01\x00\x00\x00\x00\x02";
...
int
niusb6501_packet_matches(size_t len, 
			 const void *buffer, 
			 size_t expect_len, 
			 const void *expect, 
			 const void *mask)
...
int
niusb6501_send_request(struct usb_dev_handle *usb_handle, 
		       unsigned char cmd, 
		       size_t request_len, 
		       const void *request, 
		       size_t *result_len, 
		       void *result)
{
  ...
  if (request_len > 255-(PACKET_HEADER_LEN+DATA_HEADER_LEN))
  {
    DEBUG("request too long (%d > %d bytes)\n", 
	  request_len, 
	  255-(PACKET_HEADER_LEN+DATA_HEADER_LEN));
    return -EINVAL;
  }
  ...
  /* send command */
  DEBUG("send command\n");
  status = usb_bulk_write(usb_handle,
			  EP_OUT,
			  buffer,
  ...
  /* read result */
  DEBUG("read result\n");
  status = usb_bulk_read(usb_handle,
			 EP_IN,
			 buffer,
			 sizeof buffer,
			 TIMEOUT);
...
size_t
niusb6501_list_devices(struct usb_device *devices[], 
		       size_t size)
{
...
*niusb6501_open_device(struct usb_device *device)
{
 ...
 *
 * DESCRIPTION:
 * Closes an NI USB-6501 device that has been opened with 
 *   niusb6501_open_device.
 *
 * PARAMS:
...
int
niusb6501_close_device(struct usb_dev_handle *handle)
{
...
int
niusb6501_start_counter(struct usb_dev_handle *handle)
{
  unsigned char result[8];
  size_t result_len = sizeof result;
  static const unsigned char request[] = "\x02\x20\x00\x00";
  int status;

  status = niusb6501_send_request(handle,
				  0x09,
				  sizeof request,
				  request,
				  &result_len,
...
int
niusb6501_stop_counter(struct usb_dev_handle *handle)
{
  unsigned char result[8];
  size_t result_len = sizeof result;
  static const unsigned char request[] = "\x02\x20\x00\x00";
...
int niusb6501_read_port(struct usb_dev_handle *handle, 
			unsigned char port, 
			unsigned char *value)
{
  unsigned char result[12];
  size_t result_len = sizeof result;
  unsigned char request[] = "\x02\x10\x00\x00\x00\x03\x00\x00";
  int status;
  ...
  if (! niusb6501_packet_matches(result_len, result, 12, 
...
int niusb6501_read_counter(struct usb_dev_handle *handle, 
			   unsigned long *value)
{
  unsigned char result[12];
  size_t result_len = sizeof result;
  unsigned char request[] = "\x02\x20\x00\x00";
  int status;
  ...
  if ( ! niusb6501_packet_matches(result_len, result, 12,
           "\x00\x0c\x01\x00\x00\x00\x00\x02\x00\x00\x00\x00",
...
int
niusb6501_write_port(struct usb_dev_handle *handle,
		     unsigned char port,
		     unsigned char value)
{
  unsigned char result[8];
  ...
  unsigned char request[] =
    "\x02\x10\x00\x00\x00\x03\x00\x00\x03\x00\x00\x00";
  int status;
...
int
niusb6501_write_counter(struct usb_dev_handle *handle,
			unsigned long value)
{
  unsigned char result[8];
  size_t result_len = sizeof result;
  unsigned char request[] = "\x02\x20\x00\x00\x00\x00\x00\x00";
  int status;
...
int
niusb6501_set_io_mode(struct usb_dev_handle *handle,
		      unsigned char port0,
		      unsigned char port1,
		      unsigned char port2)
{
  ...
  size_t result_len = sizeof result;
  unsigned char request[] =
    "\x02\x10\x00\x00\x00\x05\x00\x00\x00\x00\x05\x00\x00\x00\x00\x00";
  int status;
...
init_device(void)
{
  ...
  if (HANDLE == NULL)
  {
    fprintf(stderr, "ERROR: Unable to open the USB device %s\n",
	    strerror(errno));
    exit(errno);
  }

  /* ポートの状態を読み出す．
     Resource temporarily unavailable を回避するためのダミー呼び出し．
  */
  niusb6501_read_port(HANDLE, 0, &p0);
  niusb6501_read_port(HANDLE, 1, &p1);
  niusb6501_read_port(HANDLE, 2, &p2);
}
	    ...
	    unsigned char  uc_port2)
{
  ...
  i_status = niusb6501_set_io_mode(HANDLE,
				   uc_port0,
				   uc_port1,
				   uc_port2);
  if (i_status != 0)
  {
    fprintf(stderr, "ERROR: set_io_mode %s\n", strerror(errno));
    exit(i_status);
  }
}
...
read_port(unsigned char  uc_port)
{
  ...
  i_status = niusb6501_read_port(HANDLE, uc_port, &uc_value);
  if (i_status != 0)
  {
    fprintf(stderr, "ERROR read port %d: %s\n", uc_port, strerror(-i_status));
    exit(1);
  }
	   ...
	   unsigned char  uc_value)
{
  ...
  i_status = niusb6501_write_port(HANDLE, uc_port, uc_value);
  if (i_status != 0)
  {
    fprintf(stderr, "ERROR write port %d: %s\n", uc_port, strerror(-i_status));
    exit(1);
  }
}
