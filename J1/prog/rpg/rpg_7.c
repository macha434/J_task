...
main(void)
{
	...
	while (1){
		...
		if (map[player_y][player_x] == 2){
			...
			printf("秘密の鍵を手に入れた!\n");
			map[player_y][player_x] = 0;
		}else if (map[player_y][player_x] == 7){
			/* マップが見えるようになる巻物 */
			y = 0;
			while (y < 20){
				x = 0;
...
int
load_map(int map[20][80],char *fname)
{
	FILE *fp;
	int x,y;
	char buf[82];
